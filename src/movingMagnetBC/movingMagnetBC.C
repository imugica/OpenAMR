...
void Foam::movingMagnetBC::updateCoeffs()
{
    if (this->updated())
    {
        return;
    ...
    const fvPatchField<scalar>& mur =patch().lookupPatchField<volScalarField, scalar>("mur");
    ...
    if (magnetType=="Linear-square")
    {
	  	forAll(patch(),cellI)
	  	{
			...
			{
				localpsiGrad=(Mrem & patch().Sf()[cellI])/(patch().magSf()[cellI]*mur[cellI]);
			}
			else
			{
...
void Foam::movingMagnetBC::write(Ostream& os) const
{
